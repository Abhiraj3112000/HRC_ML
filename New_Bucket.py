...
def predict(nulldata):
    ...
    #nulldata = pd.read_csv("dataset.csv")
    ...
    nulldata["baseline_create_date"] = pd.to_datetime(
        nulldata.baseline_create_date)
   ...
   # if :
    ...
    nulldata['cust_number'] = nulldata['cust_number'].replace('CCCA02', "1").replace(
        ['CCU001', 'CCU013', 'CCU002'], "2").replace(['CC3411', 'CC6000'], "3").astype(int)
    ...
    nulldata.drop(['business_code', "baseline_create_date", "due_in_date", "posting_date",
                  "name_customer", "clear_date", "cust_payment_terms"], axis=1, inplace=True)
    nulldata.drop(['day_of_cleardate', "month_of_cleardate",
    ...
    model = pickle.load(open("C:/Users/KIIT/Desktop/python/HRC assignemnt/model.sav", 'rb'))
    ...
    #Final = final_result.merge(nulldata1, on=final_result.index)
    Final = pd.merge(nulldata1,final_result,left_index=True, right_index=True)

    Final['clear_date'] = pd.to_datetime(
        Final['due_in_date']) + pd.to_timedelta(Final['avg_delay'], unit='s')

    Final['avg_delay'] = Final.apply(
        lambda row: row.avg_delay//(24 * 3600), axis=1)
   
    bins = [-100,0, 15, 30, 45, 60, 100]
    labels = ['Paid earlier','0-15', '16-30', '31-45', '46-60', 'Greatar than 60']
    Final['Aging Bucket'] = pd.cut(
        Final['avg_delay'], bins=bins, labels=labels, right=False)
    print(Final['Aging Bucket'])
   ...
   # if :
    ...
    for i in range(len(doc)):
        if doc[i] in doc_list:
            a.append({"doc_id": str(int(doc[i])), "aging_bucket": str(aging[i])})
...
if __name__ == "__main__":
    ...
    clear_date = "2019-06-18"
    buisness_year = int("2019")
    doc_id = int("1929439637")
    posting_date ="2019-06-22"
    due_in_date = "2019-06-22"
    baseline_create_date = "2019-06-07"
    cust_payment_terms = "NAA8"
    converted_usd = float("11173.02")
    data = pd.DataFrame()
